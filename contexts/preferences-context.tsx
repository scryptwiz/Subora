...
import { clampUserReminderOffsets, normalizeDefaultReminderOffsets } from '@/lib/reminder-default-offsets'
...
function isReminderOffsetsSchemaError(err: { message?: string; details?: string } | null): boolean {
...
export function PreferencesProvider({ children }: { children: ReactNode }) {
    ...
    const ensureRow = useCallback(async () => {
        ...
        const selectFull = await supabase
            .from('user_settings')
            .select('display_currency, notifications_enabled, reminder_days_before, reminder_default_offsets')
            .eq('user_id', userId)
            .maybeSingle()

        let data: UserSettingsLoad | null = selectFull.data as UserSettingsLoad | null
        ...
        if (data?.display_currency) {
            ...
            const legacy =
                typeof data.reminder_days_before === 'number' ? data.reminder_days_before : 1
            setReminderOffsetsState(normalizeDefaultReminderOffsets(data.reminder_default_offsets, legacy))
            setLoading(false)
            return
        }
        ...
        if (insertError && insertError.code !== '23505' && isReminderOffsetsSchemaError(insertError)) {
            insertError = (
                await supabase.from('user_settings').insert({
                    user_id: userId,
                    display_currency: DEFAULT_CURRENCY,
    ...
    const persistPrefs = useCallback(
        async (patch: {
            ...
            const { error: e1 } = await supabase.from('user_settings').upsert(row, { onConflict: 'user_id' })
            if (!e1) return
            if (patch.reminder_default_offsets !== undefined && isReminderOffsetsSchemaError(e1)) {
                ...
                const { error: e2 } = await supabase.from('user_settings').upsert(legacy, { onConflict: 'user_id' })
                if (e2) throw e2
                return
            }
            throw e1
    ...
    const setDisplayCurrency = useCallback(
        async (code: string) => {
            ...
            try {
                ...
                const msg = upsertError instanceof Error ? upsertError.message : 'Save failed'
                setError(msg)
                throw upsertError
            }
        },
        [supabase, userId, displayCurrency, notificationsEnabled, reminderOffsets, persistPrefs]
    )
    ...
    const setNotificationsEnabled = useCallback(
        ...
        [supabase, userId, notificationsEnabled, reminderOffsets, displayCurrency, persistPrefs]
    )
    ...
    const setReminderOffsets = useCallback(
        ...
        [supabase, userId, reminderOffsets, displayCurrency, notificationsEnabled, persistPrefs]
    )
