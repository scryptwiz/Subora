...
Tables created via SQL Editor or migrations have RLS **disabled by default**. A table without RLS is fully readable and writable by anyone with the public **publishable** API key (which is bundled in clients; historically called “anon”). Run this in every migration to catch missed tables:
