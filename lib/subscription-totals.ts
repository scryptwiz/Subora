...
/** Buckets active subscriptions' monthly equivalents by ISO currency code. */
export function monthlyAmountsByCurrency(subs: Subscription[]): Record<string, number> {
...
export function sumMonthlyInDisplayCurrency(
    ...
    const byCurrency = monthlyAmountsByCurrency(subs)
    ...
    if (!snapshot || snapshot.base.toUpperCase() !== displayCurrency.toUpperCase()) {
...
export function monthlyTotalSameCurrencyOnly(subs: Subscription[], currency: string): number {
...
export function formatMultiCurrencyBreakdown(byCurrency: Record<string, number>): string {
