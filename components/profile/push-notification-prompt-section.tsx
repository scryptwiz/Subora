...
function SectionCard({ children }: { children: React.ReactNode }) {
    return (
        <View className='overflow-hidden rounded-2xl border border-[#1F1F22] bg-[#16161A]'>{children}</View>
    )
}
...
export function PushNotificationPromptSection() {
    const { permissionStatus, refresh, requestPermission, openSystemSettings, showEnableNotificationsCue } =
    ...
    const primaryLabel = permissionStatus === 'denied' ? 'Open system settings' : 'Allow notifications'
    const subtitle =
                ...
                <View className='gap-3 px-4 py-4'>
                    <View className='flex-row items-start gap-3'>
                        ...
                        <View className='min-w-0 flex-1'>
                            <Text className='font-inter-medium text-base text-white'>Enable notifications</Text>
                            <Text className='mt-1 font-inter text-xs leading-5 text-neutral-400'>{subtitle}</Text>
                        </View>
                    </View>
                    <Pressable
                    ...
                    >
                        {busy ? (
                            ...
                            <Text className='font-inter-medium text-base text-[#111111]'>{primaryLabel}</Text>
                        )}
                    </Pressable>
                </View>
            </SectionCard>
